...
@pytest.mark.parametrize("tool,version_output,impact_level", [
...
""")
    ...
    with patch.object(validator, 'workspace_root', tmp_path):
        ...
        results = validator.validate_required_files()
        assert any(r.success and "environment.yml" in r.message for r in results)
...
def test_run_validation_with_revalidation(validator):
    ...
    success_result = ValidationResult(
        True, "✅", requires_revalidation=False
    )
...
@pytest.mark.parametrize("file_type,drift_threshold", [
    ("environment.yml", 0.1),
    ("pyproject.toml", 0.2),
    (".env.template", 0.1),
    (".gitignore", 0.3),
])
def test_file_specific_drift_thresholds(validator, tmp_path, file_type, drift_threshold):
    ...
    with patch.object(validator, 'workspace_root', tmp_path):
        ...
        state_key = file_type
        assert validator.state_cache[state_key].drift_threshold == drift_threshold 
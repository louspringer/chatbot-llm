...
def generate_docs():
    ...
    for cls in classes["results"]["bindings"]:
        ...
        comment = cls.get("comment", {}).get("value", "No description available")
        md.append(f"### {name}\n")
        md.append(f"{comment}\n\n")
    ...
    for prop in props["results"]["bindings"]:
        ...
        comment = prop.get("comment", {}).get("value", "No description available")
    ...
    integration = run_query("src/chatbot_llm/queries/get_integration_details.rq")

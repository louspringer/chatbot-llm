...
class StateManager:
    ...
    def __init__(self, storage: Storage, conversation_id: str):
        ...
        Raises:
            ValueError: If conversation_id is empty or None
        """
    ...
    async def save_parameters(
        ...
        try:
            # Read current state
            data = await self._storage.read([self._conversation_id])
            state_data = data.get(self._conversation_id, {})
            if not isinstance(state_data, dict):
                state_data = {}

            # Update parameters
            state_data["id"] = self._conversation_id
            state_data["parameters"] = [dict(p) for p in parameters]

...
class ConversationData:
    ...
    def _initialize_state_machine(self) -> None:
        ...
        try:
            ...
            # Add transitions
            transitions = [
                {'trigger': 'start_auth', 'source': 'initialized', 'dest': 'authenticating', 'prepare': ['validate_transition']},
                {'trigger': 'auth_success', 'source': 'authenticating', 'dest': 'authenticated', 'prepare': ['validate_transition']},
                {'trigger': 'start_query', 'source': ['authenticated', 'idle'], 'dest': 'querying', 'prepare': ['validate_transition']},
                {'trigger': 'process_query', 'source': 'querying', 'dest': 'processing', 'prepare': ['validate_transition']},
                {'trigger': 'send_response', 'source': 'processing', 'dest': 'responding', 'prepare': ['validate_transition']},
                {'trigger': 'complete', 'source': 'responding', 'dest': 'idle', 'prepare': ['validate_transition']},
    ...
    async def create_checkpoint(self) -> None:
        ...
        self.checkpoint_data = {
            ...
            'conversation_references': self.conversation_references.copy() if self.conversation_references else {},
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any], state_manager: Optional['StateManager'] = None) -> 'ConversationData':
